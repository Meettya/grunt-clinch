...
var Compilation = function (package_config, done) {
  this.clinch = new Clinch(); // вот это стоит вынести наверх - иначе замечательный кеш, который я полировал почти неделю - не используется
...
Compilation.prototype.compile = function (package_config, done) {
  this.clinch.buldPackage(package_config, function (err, data) {
    if (err)  console.log(err); // не уверен что в случае ошиби достаточно просто отписаться в лог - может done умеет понимать ошибку?
    fs.writeFileSync(package_config.out, data); // а почему Sync?
    done && done();
  });
};
